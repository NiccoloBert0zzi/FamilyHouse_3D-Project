...
extern string name_selected_obj;
		...
		if (state == GLUT_DOWN && glutGetModifiers() == GLUT_ACTIVE_CTRL)
		{
			...
			for (int i = 0; i < Scena.size(); i++)
			{
				...
				if (ray_sphere(SetupTelecamera.position, ray_wor, Scena[i].ancora_world, raggio_sfera, &t_dist))
				{
					...
					{
						selected_obj = i;
						name_selected_obj = Scena[i].nome;
						closest_intersection = t_dist;
					}
				}
			}
			//cerco negli oggetti solo se ci sono e non è stato ancora trovato l'oggetto selezionato
			if (!ScenaObj.empty() && selected_obj == -1) {
				for (int i = 0; i < ScenaObj.size(); i++)
				{
					for (int k = 0; k < ScenaObj[i].size(); k++)
					{
						float t_dist = 0.0f;
						if (ray_sphere(SetupTelecamera.position, ray_wor, ScenaObj[i][k].ancora_world, raggio_sfera, &t_dist))
						{
							if (selected_obj == -1 || t_dist < closest_intersection)
							{
								selected_obj = i;
								name_selected_obj = ScenaObj[i][k].nome;
								closest_intersection = t_dist;
							}
						}
					}
			...
			if (selected_obj > -1)
				printf("Oggetto selezionato %d -> %s \n", selected_obj, name_selected_obj.c_str());
		}
		break;
	default:
		break;
