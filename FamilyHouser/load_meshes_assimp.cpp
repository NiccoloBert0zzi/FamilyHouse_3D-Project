...
bool loadAssImp(const char* path, vector<MeshObj>& mymesh)
{
    ...
    for (unsigned int nm = 0; nm < num_meshes; nm++)
    {
        ...
        center /= static_cast<float>(mesh->mNumVertices); // Trova il centro medio

        // Ora crea l'ancora_obj basato sul centro calcolato
        mymesh[nm].ancora_obj = vec4(center, 1.0f);
